...
import { IMsgConfig, MessageTypesEnum } from './type'
...
function provideMsgTypes() {
  // 此处可以遍历所有的enum值
  Object.values(MessageTypesEnum).forEach(
    (messageType: Partial<MessageTypesEnum>) => {
      Message[messageType] = function (options: IMsgConfig) {
